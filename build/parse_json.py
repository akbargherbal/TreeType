...
def validate_file(filepath):
    ...
    if path.suffix not in LANGUAGE_EXTENSIONS:
        return False, f"Unsupported file type: {path.suffix}. Supported: {', '.join(LANGUAGE_EXTENSIONS.keys())}"
...
def process_file(input_path, output_path=None, quiet=False):
    ...
    if not quiet:
        ...
        typeable_chars = sum(len(line.get('typing_sequence', '')) for line in json_data['lines'])
...
Supported languages:
  ...
  .tsx  -> TSX/React
    ...
    parser.add_argument(
        "input",
        nargs="+",
        help="Source file(s) or directory to parse"
    )
    parser.add_argument(
        "-o", "--output",
